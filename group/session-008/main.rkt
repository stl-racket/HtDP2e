...
(define (append-list* unprocessed processed)
    (cond
        [(empty? unprocessed) processed]
        [else (append-list* (rest unprocessed)
                            (cons (first unprocessed) processed))]))

(define (append-list left right)
    (append-list* (reverse-list left) right))

(check-expect (append-list (list 1 2 3) (list 4 5 6)) (list 1 2 3 4 5 6))

(define (flatten* unprocessed processed)
  (cond
    [(empty? unprocessed) (reverse-list processed)]
    [(cons? (first unprocessed))
     (flatten* (append-list (flatten (first unprocessed))
                            (rest unprocessed))
               processed)]
    [else (flatten* (rest unprocessed)
                    (cons (first unprocessed)
                          processed))]))

(check-expect (flatten (list 1 (list 2 (list 3 4)) 5)) (list 1 2 3 4 5))
...
(define (eliminate-duplicates lst)
  (eliminate-duplicates* (first lst) (rest lst)))

(define (eliminate-duplicates* item lst)
  (if (empty? lst) (list item)
    (eliminate-duplicates** item (eliminate-duplicates*
                                  (first lst)
                                  (rest lst)))))

(define (eliminate-duplicates** item lst)
  (cond
    [(eq? item (first lst)) lst]
    [else (cons item lst)]))

;; (define (eliminate-duplicates* item lst)
;;   (if (empty? lst) (list item)
;;     (cond
;;       [(eq? item (first (eliminate-duplicates* (first lst) (rest lst))))
;;        (eliminate-duplicates* (first lst) (rest lst))]
;;       [else (cons item (eliminate-duplicates* (first lst) (rest lst)))])))

(check-expect (eliminate-duplicates (list 1 1 2 2 2 2 2)) (list 1 2))
(check-expect (eliminate-duplicates (list 1 2 3 4)) (list 1 2 3 4))
